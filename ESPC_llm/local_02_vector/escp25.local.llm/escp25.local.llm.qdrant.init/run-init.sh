...
if [ ! -f "escp25.local.llm.qdrant.init.csproj" ]; then
    echo "Error: Please run this script from the escp25.local.llm.qdrant.init directory"
    exit 1
fi
...
if ! curl -s http://localhost:6333/collections >/dev/null 2>&1; then
    ...
    echo "  docker run -d --name qdrant-escp25 -p 6333:6333 -p 6334:6334 qdrant/qdrant"
    exit 1
fi
