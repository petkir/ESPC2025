...
using Qdrant.Client;
using espc25.local.llm.qdrant.init.Services;
using espc25.local.llm.qdrant.init.Models;

#pragma warning disable SKEXP0001

namespace espc25.local.llm.qdrant.init;
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            var sampleFilesPath = configuration["SampleFilesPath"] ?? Path.Combine("..", "espc25.local.llm", "sample_files");
    ...
    private static void ConfigureServices(IServiceCollection services, IConfiguration configuration)
    {
        ...
        var ollamaClientGlobal = new espc25.local.llm.qdrant.init.Services.OllamaEmbeddingClient(new Uri(ollamaEndpointGlobal));
        ...
        services.AddSingleton<espc25.local.llm.qdrant.init.Services.IOllamaEmbeddingClient>(sp => ollamaClientGlobal);
        ...
        services.AddSingleton<ISemanticTextMemory>(serviceProvider =>
        {
            ...
            var ollamaClient = serviceProvider.GetRequiredService<espc25.local.llm.qdrant.init.Services.IOllamaEmbeddingClient>();
            var textEmbeddingService = new espc25.local.llm.qdrant.init.Services.SkOllamaEmbeddingService(
        ...
        services.AddScoped<IQdrantInitService>(sp =>
        {
            ...
            var embedClient = sp.GetRequiredService<espc25.local.llm.qdrant.init.Services.IOllamaEmbeddingClient>();
