...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        var toolService = scope.ServiceProvider.GetRequiredService<ISemanticKernelToolService>();
        await toolService.InitializeToolsAsync(kernel);
    }
    catch (Exception ex)
    {
        var logger = scope.ServiceProvider.GetRequiredService<ILogger<Program>>();
        logger.LogWarning(ex, "Failed to initialize some Semantic Kernel tools. Some functionality may be limited.");
    }
}
